    ...
    def __init__(self, T, num_labels, ch, ch_mult, num_res_blocks, dropout):
        ...
        for i, mult in enumerate(ch_mult):
            ...
            for _ in range(num_res_blocks):
                self.downblocks.append(ConvNextBlock(in_ch=now_ch, out_ch=out_ch, tdim=tdim, dropout=dropout))
                now_ch = out_ch
                chs.append(now_ch)
            if i != len(ch_mult) - 1:
        ...
        for i, mult in reversed(list(enumerate(ch_mult))):
            ...
            for _ in range(num_res_blocks + 1):
                self.upblocks.append(ConvNextBlock(in_ch=chs.pop() + now_ch, out_ch=out_ch, tdim=tdim, dropout=dropout, attn=False))
                now_ch = out_ch
            if i != 0:
...
if __name__ == '__main__':
    ...
    # resB = ConvNextBlock(128, 256, 64, 0.1)
